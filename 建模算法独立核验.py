...
def undecidable_list(q2, q3):
    return [
        ...
        ("t3-§5-5 文献 [3]-[10] 逐条溯源", "同上；且 .paper_work/paper_07.md 的 [9]/[10] 与题面不同，按 t2 口径以题面为准"),
...
def part_F():
    ...
    for p in sorted((ROOT / ".paper_work").glob("paper_*.md")):
        docs[f".paper_work/{p.name}"] = p.read_text(encoding="utf-8")
    ...
    head = subprocess.run(["git", "-C", str(ROOT), "show", "HEAD:问题三四_独立审计.py"],
                          capture_output=True, text=True, encoding="utf-8").stdout.splitlines()
    head_line = next((i + 1 for i, l in enumerate(head) if "len(relays)" in l), None)
    ws_lines = (ROOT / "问题三四_独立审计.py").read_text(encoding="utf-8").splitlines()
...
def build_undecidable(q2, q3, boundaries):
    items = [
        ...
        {"id": "[3]-[10]", "条目": "参考文献 [3]–[10] 的系数与适用条件溯源",
         "原因": "无网络核实，且 .paper_work/paper_07.md 的 [9]/[10] 与题面冲突（已按题面纠正）",
