...
def main() -> None:
    ...
    add("## 缺口为何出现（结论）")
    add("")
    add("正缺口来自 **B 型运输无人机** 与 **中继无人机**。B 型方面，{T05,T06,T10} 两两重叠必须分置三组，"
        ...
        "中继方面，跨组保障架次必须在每个相关组各配置一套，使 K=2/K=3 的中继峰值总需求分别为 3/4，"
        "相对 2 架库存分别缺 1/2 架；其余 6 类资源均不缺。")
    add("")
    ...
    for key, value in (
        ...
        ("中继能源组件占用", "[出发, 充满)"),
        ("区间口径", "半开区间，同刻结束先于开始"),
        ("中继复制规则", "中继架次只要保障到本组任一单元，该组就独立配置一整架次；跨组时各相关组各配一套"),
