...
def main() -> None:
    ...
    add("")
    add("复制规则：中继架次保障的全部单元都落在某一任务组内时，该组需要独立配置这一整架次；"
    ...
    for k in (2, 3):
        ...
        if k == 2:
            add("**为什么 2 个组不能再省到 1 架缺口**：{T05,T06,T10} 两两重叠，2 个组最多把三者分成 2 份，"
                ...
                "枚举 63 个分区证实最小缺口为 1，且“缺口 0”的分区不存在。")
        else:
            add("**为什么 3 个组反而缺口更大**：3 个组足以把 {T05,T06,T10} 分开放置"
                ...
                "枚举 301 个分区证实最小缺口为 2，且“缺口 0/1”的分区不存在。")
        add("")
    add("### 4.3 其余 7 类资源为何不缺口")
    add("")
    ...
    for cat, why in (
        ...
        ("R", "4 个中继架次在时间上首尾衔接：RS01 [0, 5567.909)，RS03 [5567.909, 8110.431)，"
              "RS04 [7023.034, 9214.080)（与 RS03 在 7023.0–8110.4 重叠，中继机全局峰值 1、不紧张）；"
              "按“该架次保障的全部单元都在本组才复制”核算时，缺口优先分区中只有 RS01 [0, 5567.909) "
              "与 RS04 [7023.034, 9214.080) 落入 13 区组，二者时间不重叠，故每组峰值均为 1，相加 1 ≤ 库存 2"
              "（RS02、RS03 因都保障 G2 的 U3 而不满足“全部单元在本组”，不计入 G1）"),
    ...
    add("")
    add("结论：缺口不是“资源总量不够”，而是 **B 型机队规模（2 架）与 U0 内部重叠架次（T11、T16）的硬冲突**。"
        "在固定第三问方案的前提下，任何分区都无法消除这 1–2 架 B 型缺口；"
        "只能通过增配 B 型实体机（K=2 增 1 架、K=3 增 2 架）或放宽第三问的完成时间（错开 T11 与 T16）来解决。")
    add("")
    ...
    add("| --- | --- | --- | --- |")
    add("| K=2 最小缺口 | 2（B型运输机 1 + 中继机 1） | **%d**（%s） | 严格中继复制口径下中继机不缺；放宽为“触及即复制”时恰为 2 |" % (
        pri[2][1]["shortfall_total"],
        "、".join("%s×%d" % (q.CAT_NAME[c], v) for c, v in pri[2][1]["shortfall"].items() if v) or "无"))
    add("| K=3 最小缺口 | 4（B型运输机 2 + 中继机 2） | **%d**（%s） | 同上；且 K=3 最小缺口在两种口径下均为 2 |" % (
        pri[3][1]["shortfall_total"],
        "、".join("%s×%d" % (q.CAT_NAME[c], v) for c, v in pri[3][1]["shortfall"].items() if v) or "无"))
    add("| K=2 缺口优先分区 | S012 单独成组 | %s | 实算最优解中 S012 的单元 U5 与 U3(S009) 同组；强制 U5 单独成组仍是缺口 1，但配置由 25 增到 27、CV 由 0.8434 升到 0.9135 |" % (
        " / ".join("{%s}" % area_text(inst, g) for g in pri[2][0])))
    add("| K=3 缺口优先分区 | S012、S013 各自单独成组 | %s | 实算最优解中 S012 已单独成组，S013 与 S009 同组 |" % (
        " / ".join("{%s}" % area_text(inst, g) for g in pri[3][0])))
    add("| 预期分区直接复算 | — | 缺口 %d、配置 %d、CV %.6f | `{S012}/{S013}/其余 13 个服务区` 在缺口上与最优解相同，但多 2 件配置且均衡更差 |" % (
        exp3f["shortfall_total"], exp3f["resource_total"], exp3f["cv"]))
    add("")
    add("差异定位结论：单元数（7）、中继保障归属、半开区间边界三项与预期一致；"
        "**K=2 的差异来自中继架次复制判定**（严格口径 vs 触及即复制）；"
        "**K=3 的差异来自分区选择**（预期按“S012、S013 各自单独成组”排序，实算按“缺口→配置→CV”排序）。"
        "详细逐项检查见 `口径复核.md`。")
    add("")
    ...
    add("| 与旧提交对照 | 用本轮模型复算旧第三问方案（9573.284 s）同样得到 K=2 缺口 1、K=3 缺口 2，"
        "说明差异不在第三问方案版本，而在旧提交所用的资源核算规则 |")
