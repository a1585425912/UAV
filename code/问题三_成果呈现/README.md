...
> **清理提示**：数据源目录 `results/问题三_参考口径/` 已在一次删除性清理中移除（见 [../../docs/仓库清理记录.md](../../docs/仓库清理记录.md)）。本目录脚本因此需要先执行 `python code/问题三四_复现.py` 重建该目录，才能重跑下方命令；已生成的产物（`results/问题三_成果呈现/*`、`figures/问题三_成果呈现/*`）本身保留。
...
python code/问题三_成果呈现\make_q3_tables.py
python code/问题三_成果呈现\make_q3_figures.py
python code/问题三_成果呈现\make_q3_figs2.py
python code/问题三_成果呈现\weight_sweep2.py
python code/问题三_成果呈现\make_q3_report.py
```
...
本目录只记录该发现，未改动 `results/问题三_参考口径/` 的任何数值（该目录已在清理中移除，见文首提示）；后续应像第二问那样走独立复核。
