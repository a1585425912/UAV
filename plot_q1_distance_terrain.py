"""Draw separate Q1 terrain and distance bar charts with equal route spacing."""
...
TERRAIN_STEM = "raw_q1_distance_terrain"
DISTANCE_STEM = "raw_q1_distance"
...
def draw_bars(out_dir: Path, stem: str, areas: list[str], values: list[float],
              ylabel: str, color: str, ymax: float, publication: bool):
    x = np.arange(len(areas), dtype=float)
    fig, ax = plt.subplots(figsize=(7.2, 3.6))
    ax.bar(x, values, width=0.66, color=color, zorder=2)
    ax.set_ylabel(ylabel)
    ax.set_ylim(0, ymax)
    ax.set_xlim(-0.65, len(areas) - 0.35)
    ax.set_xticks(x, areas)
    ax.set_xlabel("服务区航段（O01→S）")
    ax.grid(axis="y", color="#DDE4EA", linewidth=0.6, zorder=0)
    ax.spines["left"].set_color(color)
    ax.tick_params(axis="y", colors=color)
    fig.subplots_adjust(left=0.105, right=0.975, bottom=0.17, top=0.96)
...
def main(out_dir: Path = DEFAULT_OUTPUT, publication: bool = False):
    ...
    draw_bars(out_dir, TERRAIN_STEM, areas, elevations, "航段最高 DEM 海拔 (m)",
              "#3A78A8", 620, publication)
    draw_bars(out_dir, DISTANCE_STEM, areas, distances, "单程水平距离 (km)",
              "#D18432", 9.2, publication)
    with (out_dir / f"{TERRAIN_STEM}_data.csv").open("w", encoding="utf-8-sig", newline="") as stream:
