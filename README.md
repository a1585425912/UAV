...
- [最优架次和货箱安排](results/问题一_统一航段/optimal_batches.csv)
- [全部可行候选](results/问题一_统一航段/feasible_batches.csv)
- [独立审计](results/问题一_统一航段/audit_report.txt)
- [新口径图表](figures/问题一_统一航段/)

仓库中原有的 `results/Q1_*` 结果使用**原节点表海拔和球面距离**，属于旧口径；与统一航段结果不能混用。上游几何缓存的 DEM 穿格角点完整性仍待进一步核验。
...
统一航段问题一的复现命令为 `python 问题一_统一航段复现.py`；输入哈希、参数和环境见[本次复现清单](results/问题一_统一航段/复现清单.json)。旧口径结果的复现记录仍见[原复现清单](results/复现清单.json)。
