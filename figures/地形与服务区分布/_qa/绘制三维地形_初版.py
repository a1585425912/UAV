...
SURFACE_MAX_COLS = 200        # 曲面网格上限
SURFACE_MAX_ROWS = 150
...
TERRAIN_CMAP = LinearSegmentedColormap.from_list(
    "land_elevation",
    ["#1F6F4A", "#4E9C63", "#8FBF6A", "#D8D07A", "#C0A176", "#8C6A55", "#F2EDE4"],
)
...
def style_axis_labels(ax, scene: Scene) -> None:
    ...
    ax.set_zticks(np.arange(scene.vmin, scene.vmax + 1, 100))
...
def build_3d_figure(scene: Scene, view: dict):
    ...
    rgba = light.shade(grid_z, cmap=TERRAIN_CMAP, norm=norm, vert_exag=EXAGGERATION,
                       dx=scene.dx * 1000.0, dy=scene.dy * 1000.0, blend_mode="soft")
    rgba[..., 3] = 1.0
    ...
    hill = light.hillshade(scene.crop, vert_exag=EXAGGERATION,
                           dx=scene.dx * 1000.0, dy=scene.dy * 1000.0)
    inset.imshow(hill, cmap="gray", origin="upper", alpha=0.95,
    ...
    note = ("；".join(hidden_names) + " 的地面锚点被山脊遮挡，用橙色虚线杆引出") if hidden_names \
        else "16 个地面锚点视线均通畅"
    caption = (
    ...
    label_texts = [f"{scene.nodes[i]['name']}（遮挡）" if occluded[i]
...
def build_plan_figure(scene: Scene) -> plt.Figure:
    ...
    light = LightSource(azdeg=315, altdeg=45)
    norm = Normalize(scene.vmin, scene.vmax)
    rgba = light.shade(scene.crop, cmap=TERRAIN_CMAP, norm=norm, vert_exag=EXAGGERATION,
                       dx=scene.dx * 1000.0, dy=scene.dy * 1000.0, blend_mode="soft")
    levels = np.arange(scene.vmin, scene.vmax + 1, 50.0)
    contour = ax.contour(scene.gx, scene.gy, scene.crop, levels=levels,
                         colors="#3B4A52", linewidths=0.35, alpha=0.65, zorder=3)
    ax.clabel(contour, fmt="%d", fontsize=5.4, inline=True, colors="#3B4A52",
              levels=levels[::2])
    ...
    fig.text(0.085, 0.018,
             f"30 m DEM 山体阴影 + 每 50 m 等高线；O01 为水平原点，+x 向东、+y 向北；"
...
def main() -> int:
    ...
    work_clearance = scene.clearance(view["theta"], view["azim"], lift=30.0)
